...
2. Updates its own persistent checkout of the repo (`git pull`, then a `git worktree` for the fix branch) instead of cloning from scratch each time. The branch name is deterministic — derived from the error itself, not random — so the same error always maps to the same branch.
3. Checks GitHub for whether that branch already has a PR, open or closed. This is fetched either way, but nothing is decided from it yet in code — it's handed to the AI as context, not used as a hard pre-filter.
...
7. If local tests passed, commits, pushes the branch, and opens a GitHub PR linked to the ticket.
