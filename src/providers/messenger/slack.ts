...
async function notify(config: AppConfig, ticket: Ticket, pr?: PullRequest): Promise<void> {
  ...
  if (!token) {
    throw new NotImplementedError("notify", "set SLACK_BOT_TOKEN in .env");
  }
  ...
  const res = await fetch("https://slack.com/api/chat.postMessage", {
    ...
    body: JSON.stringify({ channel: config.slackChannel, text: lines.join("\n") }),
  });
  // Slack's API returns HTTP 200 even on failure — the real status is in the body's `ok` field.
  const data = (await res.json()) as { ok: boolean; error?: string };
  if (!res.ok || !data.ok) {
    throw new Error(`Slack notify failed: ${data.error ?? res.status}`);
  }
}

export const slackMessengerProvider: MessengerProvider = { notify };
